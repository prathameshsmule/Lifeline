...
const Admin = () => {
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text(
      `Donor List for Camp: ${camps.find((c) => c._id === selectedCamp)?.name || ""}`,
      14,
      15
    );
    ...
    doc.save(`DonorList_${camps.find((c) => c._id === selectedCamp)?.name || ""}.pdf`);
  };
          ...
          <div className="table-responsive">
            <table className="table table-bordered table-hover text-center align-middle">
              ...
              <tbody>
                ...
                ) : donors.filter(d=>`${d.name} ${d.bloodGroup} ${d.phone}`.toLowerCase().includes(searchTerm.toLowerCase())).map((donor,index)=>(
                  <tr key={donor._id}>
                    <td>{index+1}</td>
                    ...
                    <td>{editDonorId===donor._id?<select className="form-select form-select-sm" name="bloodGroup" value={editForm.bloodGroup} onChange={handleEditChange}>
                      <option value="">Select</option>{["A+","A-","B+","B-","AB+","AB-","O+","O-"].map(bg=><option key={bg} value={bg}>{bg}</option>)}
                    </select>:donor.bloodGroup}</td>
                    ...
                    <td>
                      <select className="form-select form-select-sm" value={donor.remark||""} onChange={async e=>{
                        ...
                        try{
                          const token=localStorage.getItem("admin-token");
                          await axios.put(`${API_BASE}/donors/${donor._id}`,{remark:newRemark},{headers:{Authorization:`Bearer ${token}`}});
                          setDonors(prev=>prev.map(d=>d._id===donor._id?{...d,remark:newRemark}:d));
