...
const Admin = () => {
  ...
  const fetchDonors = async (token) => {
    try {
      const res = await axios.get(
        `http://www.lifelinebloodcenter.org/api/donors/camp/${selectedCamp}`,
        {
  ...
  const fetchCamps = async () => {
    try {
      const res = await axios.get("http://www.lifelinebloodcenter.org/api/camps")
      setCamps(res.data)
    } catch (err) {
      setCamps([])
    }
  ...
  const handleNewCampSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("admin-token")
      await axios.post("http://lifelinebloodcenter.org/api/camps", newCamp, {
        headers: { Authorization: `Bearer ${token}` },
      })
      setNewCamp({
        name: "",
  ...
  const handleEditSave = async (id) => {
    try {
      const token = localStorage.getItem("admin-token")
      await axios.put(`http://www.lifelinebloodcenter.org/api/donors/${id}`, editForm, {
        headers: { Authorization: `Bearer ${token}` },
      })
      setDonors((prev) =>
                        ...
                        <select
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              const token = localStorage.getItem("admin-token")
                              await axios.put(
                                `http://lifelinebloodcenter.org/api/donors/${donor._id}`,
                                { remark: newRemark },
                                {
                                  headers: { Authorization: `Bearer ${token}` },
                                }
